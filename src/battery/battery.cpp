...
{
    // Constructor for BatteryMonitor
    // Initialize the ADC pin
    _adcPin = BOARD_BAT_ADC_PIN;
    pinMode(_adcPin, INPUT);
    analogReadMilliVolts(_adcPin); // Read the initial voltage to set up the ADC
    Serial.println("Battery Monitor initialized on pin " + String(_adcPin));
    ...
    Serial.println("Battery Pin: " + String(_adcPin));
    Serial.println("Voltage Divider: " + String(_divider));
    Serial.println("ADC Attenuation: " + String(ADC_11db));
    Serial.println("ADC Resolution: " + String(12));
    Serial.println("Battery Monitor successfully initialized");
}
...
void PowerManagement::powerSaveMode()
{
    ...
    // Implement power save mode logic
    Serial.println("Entering power save mode...");
}


//Old code for the Battery class
